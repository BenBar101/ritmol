...
// NOTE: localDateFromUTC/todayUTC are called inside the arrow function — evaluated lazily at
// validation time, not captured at schema-definition time. Do not hoist to a const.
...
export const GoalSchema = z.object({
  ...
  due:     z.string().regex(/^\d{4}-\d{2}-\d{2}$/).nullable().optional(),
  // addedBy enum must match all values written by: executeCommands add_goal ("ritmol"),
  // user goal creation ("user"), and system seeding ("system").
  // If a new addedBy value is added in executeCommands or TasksTab, update this enum.
...
export const SessionSchema = z.object({
  // id is generated by `session_${crypto.randomUUID()}` — UUID hex+hyphens always match /^[\w-]+$/.
  // Do not widen this regex; session ids should never contain dots, colons, or @ symbols.
  id:       z.string().max(64).regex(/^[\w-]+$/),
...
export const ChatMessageSchema = z.object({
  ...
  // seq is a monotonically increasing counter (module-level _msgSeq in ChatTab).
  // Cap at 2_000_000 — far above the practical maximum (1000 messages × 2 roles).
  seq:     z.number().int().min(0).max(2_000_000).optional(),
})
...
// NOTE: localDateFromUTC is called inside the arrow function — evaluated lazily at
// validation time, not captured at schema-definition time. Do not hoist to a const.
...
export const SyncPayloadSchema = z.object({
  ...
  jv_xp:                  z.number().int().min(0).max(10_000_000).optional(),
